// 桥接层改编自 Cowart (MIT, Copyright 2026 Twox) https://github.com/zhongerxin/Cowart
...
export function inlineWidget({
  ...
  js = "",
  initialDisplayMode = "",
  cssPlaceholder = "/* __SKILLDECK_WIDGET_CSS__ */",
  jsPlaceholder = "/* __SKILLDECK_WIDGET_JS__ */",
}) {
...
function injectMcpHostBridge(html, { initialDisplayMode = "" } = {}) {
  const bridge = [
    '<script id="skilldeckInitialDisplayMode">',
    `window.__SKILLDECK_INITIAL_DISPLAY_MODE__=${JSON.stringify(initialDisplayMode)};`,
    "</script>",
    '<script id="skilldeckMcpAppsBundle">',
    escapeInlineScript(mcpAppsGlobalScript()),
    "</script>",
    '<script id="skilldeckMcpHostBridge">',
    mcpHostBridgeScript(),
    "</script>",
  ].join("\n");
...
function mcpAppsGlobalScript() {
  ...
  cachedMcpAppsGlobalScript = [
    ...
    ";globalThis.__SKILLDECK_MCP_APPS__={",
...
function mcpHostBridgeScript() {
  ...
  const apps = globalThis.__SKILLDECK_MCP_APPS__;
  ...
  function toBridgeError(error) {
    ...
    return new Error(String(error || "SkillDeck host bridge is unavailable."));
  }
  ...
  async function waitForReady(app) {
    if (app?.ready) {
      await withTimeout(app.ready, 4000, "SkillDeck host bridge did not become ready.");
    }
    if (globalThis.__SKILLDECK_MCP_HOST_ERROR__) {
      throw toBridgeError(globalThis.__SKILLDECK_MCP_HOST_ERROR__);
    }
  }

  function installSkillDeckApi(app) {
    const api = window.skilldeckMcp || {};
    window.skilldeckMcp = api;
    ...
    api.callServerTool = async (request, options) => {
      try {
        ...
        return await withTimeout(
          ...
          "SkillDeck server tool call timed out.",
        );
      } catch (error) {
        throw toBridgeError(error);
      }
  ...
  try {
    mcpApp = new apps.App(
      { name: "skilldeck", version: "0.1.6" },
      { availableDisplayModes: ["inline", "fullscreen"] },
      { autoResize: true },
    );
    globalThis.__SKILLDECK_MCP_APP__ = mcpApp;
    installSkillDeckApi(mcpApp);
    ...
    mcpApp.ready = mcpApp.connect()
      .then(() => {
        ...
        const initialMode = window.__SKILLDECK_INITIAL_DISPLAY_MODE__;
      ...
      .catch((error) => {
        globalThis.__SKILLDECK_MCP_HOST_ERROR__ = error;
      });
  } catch (error) {
    globalThis.__SKILLDECK_MCP_HOST_ERROR__ = error;
  }
